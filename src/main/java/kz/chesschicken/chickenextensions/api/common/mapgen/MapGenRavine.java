    ...
    protected void generateRavine(long par1, int par3, int par4, byte[] par5ArrayOfByte, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17)
    {
        ...
        for (; par15 < par16; ++par15)
        {
            ...
            if (var53 || var19.nextInt(4) != 0)
            {
                ...
                {
                    int var55 = MathHelper.floor(par6 - var54) - par3 * 16 - 1;
                    int var35 = MathHelper.floor(par6 + var54) - par3 * 16 + 1;
                    int var56 = MathHelper.floor(par8 - var30) - 1;
                    int var37 = MathHelper.floor(par8 + var30) + 1;
                    int var57 = MathHelper.floor(par10 - var54) - par4 * 16 - 1;
                    int var39 = MathHelper.floor(par10 + var54) - par4 * 16 + 1;
                                ...
                                if (var59 * var59 + var45 * var45 < 1.0D)
                                {
                                    for (int var49 = var37 - 1; var49 >= var56; --var49)
                                    {
                                        ...
                                        if ((var59 * var59 + var45 * var45) * (double)this.field_35627_a[var49] + var50 * var50 / 6.0D < 1.0D)
                                        {
                                            ...
                                            if (var52 == BlockBase.GRASS.id)
                                            {
                                                var48 = true;
                                            }

                                            if (var52 == BlockBase.STONE.id || var52 == BlockBase.DIRT.id || var52 == BlockBase.GRASS.id)
                                            {
                                                if (var49 < 10)
                                                {
                                                    par5ArrayOfByte[var47] = (byte)BlockBase.FLOWING_LAVA.id;
                                                }
                                                else
                                                {
                                                    par5ArrayOfByte[var47] = 0;

                                                    if (var48 && par5ArrayOfByte[var47 - 1] == BlockBase.DIRT.id)
                                                    {
                                                        par5ArrayOfByte[var47 - 1] = this.level.getBiomeSource().getBiome(var41 + par3 * 16, var44 + par4 * 16).topTileId;
                                                    }
                                                }
                                            }
                                        }
