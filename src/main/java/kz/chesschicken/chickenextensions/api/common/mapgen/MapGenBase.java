...
public class MapGenBase {
    ...
    public void generate(LevelSource chunkIO, Level level, int par3, int par4, byte[] par5ArrayOfByte)
    {
        int var6 = this.range;
        this.level = level;
        ...
        for (int var11 = par3 - var6; var11 <= par3 + var6; ++var11)
        {
            for (int var12 = par4 - var6; var12 <= par4 + var6; ++var12)
            {
                ...
                this.recursiveGenerate(level, var11, var12, par3, par4, par5ArrayOfByte);
            }
        }
    }

    protected void recursiveGenerate(Level level, int x1, int x2, int x3, int x4, byte[] x5) {}
}
